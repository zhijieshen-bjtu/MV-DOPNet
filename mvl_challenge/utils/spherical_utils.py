...
def phi_coords2xyzTorch(phi_coords):
    """
    Returns 3D bearing vectors (on the unit sphere) from phi_coords
    """
    # No batch size
    W = phi_coords.shape[-1]
    u = torch.linspace(0, W - 1, W, device=phi_coords.device)
    theta_coords = (2 * math.pi * u / W) - math.pi
    ...
    return torch.stack((bearings_x, bearings_y, bearings_z), dim=-1)
...
def phi_coords2xyz(phi_coords):
    ...
    #print('theta_coords ',theta_coords)
