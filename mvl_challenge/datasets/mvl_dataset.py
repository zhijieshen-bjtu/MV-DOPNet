...
def iter_mvl_room_scenes(model, dataset: MVLDataset):
    ...
    for room_scene in dataset.list_rooms:
        ...
        # ratio_sum = 0
        # for ly in list_ly:
        #     ratio_sum = ratio_sum + ly.ratio
        # avg_ratio = float(ratio_sum / len(list_ly))

        #不要平均，要精确的

        # [ly.set_ratio(ratio=ly.ratio) for ly in list_ly]
...
def iter_mvl_room_scenes_train(dataset: MVLDataset):
    """
    Creates a generator which yields a list of layout from a defined
    MVL dataset and estimates layout in it.
    """
    ...
    for room_scene in dataset.list_rooms:
        ...
        #filter_out_noisy_layouts(list_ly=list_ly, max_room_factor_size=cfg.runners.mvl.max_room_factor_size)
        
        yield list_ly

        
class MVLDataset_val:
    def __init__(self, cfg, mode):
        logging.info("Initializing MVL Dataset for VAL...")
        self.cfg = cfg
        self.mode = mode
        self.set_paths()
        logging.info("MVL Dataset successfully initialized")
    ...
    def set_paths(self):
        ...
        #! List of scene names
        assert os.path.exists(
            self.cfg.runners.valid_iou.scene_list
        ), f"Not found scene list {self.cfg.runners.valid_iou.scene_list}"
        logging.info(f"Scene list: {self.cfg.runners.valid_iou.scene_list}")
        self.data_scenes = json.load(open(self.cfg.runners.valid_iou.scene_list))
        self.list_rooms = list(self.data_scenes.keys())
        self.list_frames = [fr for fr in self.data_scenes.values()]
        self.list_frames = [item for sublist in self.list_frames for item in sublist]
    ...
    def set_geom_info(layout: Layout, geom):
        ...
        layout.pose.idx = layout.idx
        layout.camera_height = geom["cam_h"]
        layout.ratio = geom["ratio"]
        
class MVLDataset_train:
    def __init__(self, cfg, mode):
        logging.info("Initializing MVL Dataset for VAL...")
        self.cfg = cfg
        self.mode = mode
